...
class CompetitionUpdatesScreen extends StatefulWidget {
  final String IT_or_nonIT;

  const CompetitionUpdatesScreen(this.IT_or_nonIT, {Key? key})
      : super(key: key);

  @override
  State<CompetitionUpdatesScreen> createState() =>
      _CompetitionUpdatesScreenState(IT_or_nonIT);
}

class _CompetitionUpdatesScreenState extends State<CompetitionUpdatesScreen> {
  final String IT_or_nonIT;
  late bool accessToken = false;
  _CompetitionUpdatesScreenState(this.IT_or_nonIT);
  ...
  CollectionReference get _product =>
      FirebaseFirestore.instance.collection("${IT_or_nonIT}CompetitionUpdates");
  ...
  fetchUserProfileData() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("LOADING.. user profile $e")));
    }
  }
            ...
            if (docs != null) {
              return ListView.builder(
                ...
                itemBuilder: (context, rowIndex) {
                  ...
                  } else if (docs[rowIndex]['company'] == selectedCompany) {
                    return addCard(
